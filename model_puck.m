...
p_c_hat = @(r, phi) integral2(@(ro, nu) ...
	...
	0, r, 0, 2*pi);
p_c =@(r, phi) p_b(r, phi) - p_a_hat(r, phi);

p_d = @(r, phi) 1 - delta.^2 .* sin(phi).^2 .* p_b(r, phi) .* p_c(r, phi);

p_parall =@(r, phi) p_a(r, phi) / p_d(r, phi);
p_omega = 2*(p_a_hat + (delta^2 * cos(phi)^2 * p_a * p_b * p_c_hat/p_d));
p_orth = delta * p_a * p_b / p_d;
J = f * N / m;
